...
function validateApiKey(
  ...
  if (!apiKey) {
    return {
      valid: false,
      error: 'API access disabled: API_KEY not configured',
    };
  }
  ...
  switch (url.pathname) {
    ...
    default: {
      ...
      if (isValidUUID(pathUUID)) {
        ...
        const isValidUser = validUUIDs.some((uuid) => uuid.toLowerCase() === normalizedUUID);
 ...
 * 处理配置路径请求
 */
function handleConfigPath(_userID: string, _hostName: string): Response {
...
export { createMuxSession, MuxSession } from './handlers/mux-session';
export {
  // 提供者
  createUUIDManager,
  // UUID 验证器
